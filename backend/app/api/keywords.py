...
# 검색량 캐시 (5분간 유지)
_search_cache: Dict[str, tuple[int, float]] = {}
...
async def get_search_volume(keyword: str, retry_count: int = 2) -> Optional[int]:
    """
    ...
    for attempt in range(retry_count + 1):
        try:
            ...
            if attempt < retry_count:
                print(f"블로그 발행량 조회 타임아웃 ({keyword}), 재시도 {attempt + 1}/{retry_count}")
                await asyncio.sleep(0.5)  # 재시도 전 대기
                continue
            print(f"블로그 발행량 조회 타임아웃 ({keyword}) - 최종 실패")
            return None
        except httpx.TimeoutException:
            if attempt < retry_count:
                print(f"블로그 발행량 조회 HTTP 타임아웃 ({keyword}), 재시도 {attempt + 1}/{retry_count}")
                await asyncio.sleep(0.5)
                continue
            print(f"블로그 발행량 조회 HTTP 타임아웃 ({keyword}) - 최종 실패")
            return None
        except httpx.HTTPStatusError as e:
            error_msg = f"HTTP {e.response.status_code}"
            try:
                error_text = e.response.text[:200]  # 더 긴 에러 메시지 확인
                error_msg += f": {error_text}"
            except:
                pass
            print(f"블로그 발행량 조회 HTTP 에러 ({keyword}): {error_msg}")
            
            # HTTP 429 (Too Many Requests) 에러는 재시도
            if e.response.status_code == 429:
                if attempt < retry_count:
                    wait_time = (attempt + 1) * 1.0  # 1초, 2초, 3초 대기
                    print(f"블로그 발행량 조회 API 제한 ({keyword}), {wait_time}초 후 재시도 {attempt + 1}/{retry_count}")
                    await asyncio.sleep(wait_time)
                    continue
                print(f"블로그 발행량 조회 API 제한 ({keyword}) - 최종 실패")
                return None
            ...
            if attempt < retry_count:
                print(f"블로그 발행량 조회 실패 ({keyword}), 재시도 {attempt + 1}/{retry_count}: {str(e)}")
                await asyncio.sleep(0.5)
                continue
...
async def get_weekly_search_volume(keyword: str, retry_count: int = 2) -> tuple[Optional[int], bool]:
    """
    네이버 데이터랩 API를 통해 실제 일주일 검색량 조회
    캐싱을 사용하여 안정성 향상, 재시도 로직 포함
    
    Returns:
        (volume, is_from_datalab): 검색량과 데이터랩 API에서 온 데이터인지 여부
    """
    # 캐시 확인 (캐시는 이전 호출에서 저장된 것이므로 데이터 소스 추적 불가)
    current_time = time.time()
    if keyword in _search_cache:
        cached_volume, cache_time = _search_cache[keyword]
        if current_time - cache_time < CACHE_DURATION:
            print(f"[검색량] 캐시에서 반환 ({keyword}): {cached_volume} (데이터 소스 불명확)")
            # 캐시된 데이터는 데이터 소스를 알 수 없으므로 추정값으로 간주
            return (cached_volume, False)
    
    if not settings.naver_client_id or not settings.naver_client_secret:
        print(f"[검색량] API 키 없음 ({keyword}), 폴백으로 전환")
        volume = await _get_weekly_search_volume_fallback(keyword)
        return (volume, False)  # 폴백이므로 데이터랩 아님
    ...
    for attempt in range(retry_count + 1):
        try:
            ...
            async with httpx.AsyncClient(timeout=httpx.Timeout(8.0, connect=5.0)) as client:
                ...
                # 응답 데이터에서 일주일 검색량 계산
                # 네이버 데이터랩은 상대 비율(ratio)을 제공하므로, 평균 비율을 계산
                weekly_volume = 0
                if "results" in data and len(data["results"]) > 0:
                    result = data["results"][0]
                    if "data" in result and len(result["data"]) > 0:
                        # 각 날짜별 ratio 합산 (상대 비율의 합)
                        ratios = []
                        ...
                        if ratios:
                            ...
                            # 상대 비율을 실제 검색량으로 변환 (대략적인 추정)
                            # 데이터랩의 ratio는 최대값을 100으로 정규화한 값
                            # 실제 검색량 추정: 평균 비율 * 1000 (대략적인 스케일)
                            weekly_volume = int(avg_ratio * len(ratios) * 10)
                            print(f"[검색량] 데이터랩에서 계산된 검색량 ({keyword}): {weekly_volume}")
                
                # 검색량이 0이거나 너무 작으면 폴백으로 전환
                if weekly_volume == 0 or weekly_volume < 1:
                    print(f"[검색량] 검색량이 0 또는 너무 작음 ({keyword}): {weekly_volume}, 폴백으로 전환")
                    volume = await _get_weekly_search_volume_fallback(keyword)
                    return (volume, False)  # 폴백이므로 데이터랩 아님
                
                # 캐시에 저장
                _search_cache[keyword] = (weekly_volume, current_time)
                print(f"[검색량] 데이터랩 API 조회 성공 ({keyword}): {weekly_volume}")
                return (weekly_volume, True)  # 데이터랩 API에서 온 실제 데이터
            ...
            if e.response.status_code == 400:
                print(f"[검색량] 데이터랩 API 400 에러 ({keyword}): 키워드가 데이터랩에 없을 수 있습니다. 폴백으로 전환.")
                # 폴백: 블로그 검색 API 사용
                volume = await _get_weekly_search_volume_fallback(keyword)
                return (volume, False)  # 폴백이므로 데이터랩 아님
            ...
            if e.response.status_code == 429:
                if attempt < retry_count:
                    wait_time = (attempt + 1) * 1.0  # 1초, 2초 대기
                    print(f"[검색량] 데이터랩 API 429 에러 ({keyword}), {wait_time}초 후 재시도 {attempt + 1}/{retry_count}")
                    await asyncio.sleep(wait_time)
                    continue
                print(f"[검색량] 데이터랩 API 429 에러 ({keyword}): API 요청 제한 초과. 폴백으로 전환.")
                volume = await _get_weekly_search_volume_fallback(keyword)
                return (volume, False)  # 폴백이므로 데이터랩 아님
            
            # 다른 HTTP 에러는 재시도
            if attempt < retry_count:
                print(f"[검색량] 데이터랩 API HTTP 에러 ({keyword}): {e.response.status_code} - {error_text}, 재시도 {attempt + 1}/{retry_count}")
                await asyncio.sleep(0.5)
                continue
            
            print(f"[검색량] 데이터랩 API HTTP 에러 ({keyword}): {e.response.status_code} - {error_text}, 폴백으로 전환")
            volume = await _get_weekly_search_volume_fallback(keyword)
            return (volume, False)  # 폴백이므로 데이터랩 아님
            
        except (httpx.TimeoutException, httpx.RequestError, asyncio.TimeoutError) as e:
            if attempt < retry_count:
                print(f"[검색량] 데이터랩 API 타임아웃/에러 ({keyword}): {str(e)}, 재시도 {attempt + 1}/{retry_count}")
                await asyncio.sleep(0.5)
                continue
            print(f"[검색량] 데이터랩 API 타임아웃/에러 ({keyword}): {str(e)}, 폴백으로 전환")
            volume = await _get_weekly_search_volume_fallback(keyword)
            return (volume, False)  # 폴백이므로 데이터랩 아님
            
        except Exception as e:
            if attempt < retry_count:
                print(f"[검색량] 일주일 검색량 조회 실패 ({keyword}), 재시도 {attempt + 1}/{retry_count}: {str(e)}")
                await asyncio.sleep(0.5)
                continue
    ...
    # 모든 재시도 실패 시 폴백
    print(f"[검색량] 모든 재시도 실패 ({keyword}), 폴백으로 전환")
    volume = await _get_weekly_search_volume_fallback(keyword)
    return (volume, False)  # 폴백이므로 데이터랩 아님
...
async def _get_weekly_search_volume_fallback(keyword: str) -> Optional[int]:
    ...
    try:
        ...
        if total_volume is None:
            ...
            # 캐시에 저장
            current_time = time.time()
            _search_cache[keyword] = (weekly_estimate, current_time)
        ...
        if total_volume == 0:
            ...
            current_time = time.time()
            _search_cache[keyword] = (weekly_estimate, current_time)
            print(f"[검색량 폴백] 검색량 0, 최소값 반환 ({keyword}): {weekly_estimate}")
            return weekly_estimate
        
        # 전체 검색량을 기반으로 일주일 검색량 추정
        if total_volume < 100:
        ...
        print(f"[검색량 폴백] 추정값 반환 ({keyword}): {weekly_estimate} (전체 검색량: {total_volume})")
        return weekly_estimate
...
async def get_keyword_suggestions(
    ...
    try:
        ...
        try:
            ...
            for i, result in enumerate(search_results):
                if isinstance(result, Exception):
                    ...
                    volume, is_from_datalab = result
                    search_volumes[i] = volume
                    search_sources[i] = "api" if is_from_datalab else "fallback_or_estimated"
                else:
                    # 예상치 못한 형식
                    search_volumes[i] = None
            ...
            for i, item in enumerate(keyword_list):
                try:
                    ...
                    # 요청 간 간격 (네이버 API 제한 고려) - 300ms로 증가
                    if i < len(keyword_list) - 1:
                        await asyncio.sleep(0.3)  # 300ms 대기 (API 제한 방지)
                except Exception as e:
                    print(f"블로그 발행량 조회 실패 ({item['keyword']}): {str(e)}")
                    blog_counts[i] = None
                    # 실패해도 다음 키워드 계속 조회
                    if i < len(keyword_list) - 1:
        ...
        # 검색량 기준 내림차순 정렬
        # 실제 검색량이 있는 키워드를 우선하고, None인 키워드는 뒤로
        suggestions.sort(key=lambda x: (x.search_volume is not None, x.search_volume or 0), reverse=True)
...
async def analyze_keyword(request: KeywordAnalysisRequest) -> KeywordAnalysisResponse:
    ...
    try:
        ...
        search_volume, blog_count = await asyncio.gather(
            search_volume_task,
            blog_count_task,
        ...
        if isinstance(search_volume, Exception):
            print(f"일주일 검색량 조회 실패 ({request.keyword}): {type(search_volume).__name__}: {str(search_volume)}")
            if isinstance(search_volume, asyncio.TimeoutError):
                print(f"  -> 타임아웃 에러")
            search_volume = None
        ...
        # 경쟁 강도 계산 (검색량 기준)
        volume_for_competition = search_volume if search_volume else 2500
        if volume_for_competition > 10000:
            competition = "high"
        elif volume_for_competition > 1000:
            competition = "medium"
        else:
            competition = "low"
